...
import { trackCategoryInteraction } from './recommendationService';
...
export const toggleWishlist = async (productId: string, productData: any) => {
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      await setDoc(wishlistRef, sanitizeForFirestore({
        ...productData,
        addedAt: serverTimestamp()
      }));
      
      // Track this category with higher weight
      if (productData.categoryId) {
        trackCategoryInteraction(productData.categoryId, 5);
      }
