...
export default function AppShell({ children }: AppShellProps) {
  ...
  const hideLayout = isProductPage || isCompleteProfilePage;

  return (
    <div className={cn("min-h-screen bg-transparent font-bengali", hideLayout ? "bg-white" : "")}>
      <DynamicBackground />
      <div className={cn("flex flex-col lg:flex-row min-h-screen", hideLayout ? "flex-col" : "")}>
        {/* Desktop Sidebar */}
        {!hideLayout && (
          <aside className="hidden lg:flex w-64 bg-white/40 backdrop-blur-xl border-r border-white/20 flex-col sticky top-0 h-screen">
            <div className="p-6">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!hideLayout && (
            <header className="sticky top-0 z-40 bg-white/60 backdrop-blur-xl border-b border-white/20 shadow-sm px-4 lg:px-8 h-16 flex items-center justify-between shrink-0">
          ...
          {/* Main Content Area */}
          <main className={cn(
            "flex-1 w-full font-bengali",
            !hideLayout ? "pb-20 lg:pb-8 p-4 lg:p-8 max-w-7xl mx-auto" : ""
          )}>
            {children}
          </main>
        </div>
      ...
      {!isCompleteProfilePage && <SupportChat externalOpen={isSupportChatOpen} onClose={() => setSupportChatOpen(false)} />}
      ...
      {!hideLayout && (
        <nav className="fixed lg:hidden bottom-0 left-0 right-0 z-40 bg-white/60 backdrop-blur-xl border-t border-white/20 px-6 h-16 flex items-center justify-between">
