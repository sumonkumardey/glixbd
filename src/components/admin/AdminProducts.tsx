...
import { formatPrice, compressImage, sanitizeForFirestore } from '@/src/lib/utils';
...
export default function AdminProducts() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProduct) {
        await updateDoc(doc(db, 'products', editingProduct.id), sanitizeForFirestore({
          ...productData,
          updatedAt: serverTimestamp()
        }));
        console.log("✅ Product updated successfully!");
      } else {
        await addDoc(collection(db, 'products'), sanitizeForFirestore({
          ...productData,
          views: 0,
          rating: 4.5,
          reviewCount: 0,
