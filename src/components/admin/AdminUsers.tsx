...
import { cn, sanitizeForFirestore } from '@/src/lib/utils';
...
export default function AdminUsers() {
  ...
  const toggleBlock = async (user: UserProfile) => {
    try {
      ...
      await updateDoc(userRef, sanitizeForFirestore({ isBlocked: !user.isBlocked }));
      fetchUsers();
    } catch (err) {
      console.error(err);
  ...
  const toggleAdmin = async (user: UserProfile) => {
    ...
    try {
      if (isAdmin) {
        ...
        await setDoc(doc(db, 'admins', user.uid), sanitizeForFirestore({
          email: user.email,
          name: user.name,
  ...
  const addAdminEmail = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'admin_emails', emailLower), sanitizeForFirestore({
