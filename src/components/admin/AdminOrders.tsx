...
export default function AdminOrders() {
  ...
  const handleSendToSteadfast = async (order: any) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await addDoc(collection(db, `orders/${order.id}/tracking`), sanitizeForFirestore({
          status: 'shipped',
  ...
  const updateStatus = async (orderId: string, newStatus: string) => {
    ...
    try {
      ...
      // Update order status
      await updateDoc(docRef, sanitizeForFirestore({ status: newStatus }));
      
      // Add tracking history event
      await addDoc(collection(db, `orders/${orderId}/tracking`), sanitizeForFirestore({
        status: newStatus,
        statusBn: details.bn,
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-sm font-bold space-y-1">
                    ...
                    <p className="text-primary font-black flex items-center gap-2">
                      ...
                      <FraudScoreBadge phone={order.shippingAddress?.phone} />
                    </p>
