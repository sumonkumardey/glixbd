...
import { formatPrice, cn, sanitizeForFirestore } from '@/src/lib/utils';
...
export default function CheckoutScreen() {
  ...
  const [paymentSettings, setPaymentSettings] = useState({
    bkashNumber: '01622433971',
    nagadNumber: '01622433971',
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      try {
        ...
        await setDoc(orderRef, sanitizeForFirestore(orderData));
        ...
        if (walletAmountUsed > 0 && auth.currentUser) {
          ...
          await runTransaction(db, async (transaction) => {
            ...
            transaction.update(userRef, sanitizeForFirestore({ walletBalance: newBalance }));
            
            // Log transaction
            const transRef = doc(collection(db, `users/${auth.currentUser!.uid}/walletTransactions`));
            transaction.set(transRef, sanitizeForFirestore({
        ...
        await addDoc(collection(db, `orders/${generatedOrderNum}/tracking`), sanitizeForFirestore({
          status: 'pending',
