...
export default function ProductDetailScreen() {
    ...
    async function fetchProduct() {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          try {
            await updateDoc(productRef, sanitizeForFirestore({
              views: increment(1)
            }));
  ...
  const handleToggleLike = async (review: Review) => {
    ...
    try {
      if (currentlyLiked) {
        ...
        await updateDoc(reviewRef, sanitizeForFirestore({ likesCount: increment(-1) }));
        setUserLikes(prev => ({ ...prev, [review.id]: false }));
      } else {
        await setDoc(likeRef, sanitizeForFirestore({
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await addDoc(collection(db, 'reviews'), sanitizeForFirestore({
        productId: id,
        userId: auth.currentUser.uid,
        userName: auth.currentUser.displayName || 'সম্মানিত গ্রাহক',
